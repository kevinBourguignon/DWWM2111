...
<article id="post-<?php the_ID(); ?>" <?php post_class('inner-service'); ?>>
  <div class="post-main-box wow zoomInDown delay-1000" data-wow-duration="2s">
    ...
    <?php }else if($vw_automobile_lite_theme_lay == 'Center'){ ?>
      <div class="new-text">
        <h2 class="section-title"><a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php echo the_title_attribute(); ?>"><?php the_title();?><span class="screen-reader-text"><?php the_title(); ?></span></a></h2>
        <?php if( get_theme_mod( 'vw_automobile_lite_toggle_postdate',true) != '' || get_theme_mod( 'vw_automobile_lite_toggle_author',true) != '' || get_theme_mod( 'vw_automobile_lite_toggle_comments',true) != '' || get_theme_mod( 'vw_automobile_lite_toggle_time',true) != '') { ?>
          <div class="metabox">
            ...
            <?php if(get_theme_mod('vw_automobile_lite_toggle_author',true)==1){ ?>
              <span class="entry-author"><span><?php echo esc_html(get_theme_mod('vw_automobile_lite_meta_field_separator'));?></span><i class="<?php echo esc_attr(get_theme_mod('vw_automobile_lite_toggle_author_icon','far fa-user')); ?>"></i><a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' )) ); ?>"><?php the_author(); ?></a></span>
            <?php } ?>

            <?php if(get_theme_mod('vw_automobile_lite_toggle_comments',true)==1){ ?>
              <span class="entry-comments"><span><?php echo esc_html(get_theme_mod('vw_automobile_lite_meta_field_separator'));?></span><i class="<?php echo esc_attr(get_theme_mod('vw_automobile_lite_toggle_comments_icon','fas fa-comments')); ?>"></i><?php comments_number( __('0 Comments','vw-automobile-lite'), __('0 Comments','vw-automobile-lite'), __('% Comments','vw-automobile-lite')); ?></span>
            <?php } ?>

            <?php if(get_theme_mod('vw_automobile_lite_toggle_time',true)==1){ ?>
              <span><?php echo esc_html(get_theme_mod('vw_automobile_lite_meta_field_separator'));?></span><i class="<?php echo esc_attr(get_theme_mod('vw_automobile_lite_toggle_time_icon','far fa-clock')); ?>"></i><span class="entry-time"><?php echo esc_html( get_the_time() ); ?></span>
            <?php } ?>
          </div>
        <?php } ?>
       <?php if( get_theme_mod( 'vw_automobile_lite_featured_image_hide_show',true) != '') { ?>
          <div class="box-image">
            <?php the_post_thumbnail(); ?>
          </div>
         <?php } ?>
